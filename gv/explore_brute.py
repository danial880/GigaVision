...
class Explore:
    ...
    def get_predictions(self, resize_width, detection_model, slicee_width, img_path, resize_height, slicee_height):
    ...
    def save_json(self, slicee, resize, csv_list):
        slice_dimension = str(self.ss.s_w[slicee]) + 'x' + str(self.ss.s_h[slicee])
        resize_dimension = str(self.ss.r_w[resize]) + 'x' + str(self.ss.r_h[resize])
        json_name = slice_dimension + '_' + resize_dimension + '_' + self.outfile
    ...
    def log_time_results(self, elapsed_time_resize, results, resize):
        if not self.set_time:
            self.set_time = True
            total_resize_time = elapsed_time_resize * len(self.ss.r_h) * len(self.ss.s_h) * len(self.ss.r_w) \
                * len(self.ss.s_w)
                    ...
                    for resize_height in range(len(self.ss.r_h)):
                        for resize_width in range(len(self.ss.r_w)):
                            ...
                            for img_path in tqdm(self.files):
                                result = self.get_predictions(resize_width, detection_model, slicee_width, img_path,
                            ...
                            if csv_list:
                                ...
                                results = self.save_parameters(results, resize_width, slicee_width, resize_height,
                                                               slicee_height)
